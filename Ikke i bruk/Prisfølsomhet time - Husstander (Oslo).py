...
def finne_husstander():
    for index, rad in data_answer.iterrows():
        if (
                ...
                rad["Q21"] == 6 and        # 1 = Under 300 000 kr, 2 = 300 000 - 499 999, 3 = 500 000 -799 999, 4 = 800 000 - 999 999, 5 = 1 000 000 - 1 499 999, 6 = 1 500 000 eller mer, 7 = Vil ikke oppgi, 8 = Vet ikke
...
def direkte_prisfolsomhet_time(test_liste_husstander, data_demand, data_price_update, data_households, Blindern_Temp_t4t):
...
def lin_log_prisfolsomhet_pris_t4t(test_liste_husstander, data_demand, data_price_update, data_households, Blindern_Temp_t4t):
...
def log_lin_prisfolsomhet_pris_t4t(test_liste_husstander,data_demand,data_price_update,data_households, Blindern_Temp_t4t):
    start_dato = '2021-04-01'
    end_dato = '2022-03-31'
    start_dato = pd.to_datetime(start_dato)
    end_dato = pd.to_datetime(end_dato)
...
def log_log_prisfolsomhet_t4t(test_liste_husstander, data_demand, data_price_update, data_households, Blindern_Temp_t4t):
    ...
    for ID in test_liste_husstander:
        ...
        price_data['Hour'] = price_data['Hour'].astype(int)
        price_filtered = price_data[(price_data['Date'] >= start_dato) & (price_data['Date'] <= end_dato)]
        price_filtered.loc[:, 'Price_NOK_kWh'] = price_filtered['Price_NOK_kWh'].apply(lambda x : x if x > 0 else 0.01) # Dette skal fikset prisen, om den er negativ
