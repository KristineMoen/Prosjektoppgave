...
def finne_husstander():
    for index, rad in data_answer.iterrows():
        if (
                rad["Q_City"] in [5]  and    # 4 = Oslo, 2 = Lillestrøm, 1 = Bærum
...
def price_responsitivity(liste_husstander, data_demand, data_price_update, data_households, Blindern_Temp_t4t):
    ...
    # ------------ PRINTER UT KWH PER HUSHOLDNING SÅ DEN E SAMMENLIGNBAR ----------------
    #print('Average',model.params['C(Price_Group, Treatment(reference="Before_ref"))[T.After_ref]']/len(liste_husstander))
